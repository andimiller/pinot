...
public class DistinctCountULLAggregationFunction extends BaseSingleInputAggregationFunction<UltraLogLog, Double> {
  ...
  public void aggregate(int length, AggregationResultHolder aggregationResultHolder,
    ...
    if (storedType == DataType.BYTES) {
      ...
      try {
        UltraLogLog ultraLogLog = aggregationResultHolder.getResult();
        if (ultraLogLog != null) {
          for (int i = 0; i < length; i++) {
            ultraLogLog.add(ObjectSerDeUtils.ULTRA_LOG_LOG_OBJECT_SER_DE.deserialize(bytesValues[i]));
          }
        } else {
          ultraLogLog = ObjectSerDeUtils.ULTRA_LOG_LOG_OBJECT_SER_DE.deserialize(bytesValues[0]);
          aggregationResultHolder.setValue(ultraLogLog);
    ...
    UltraLogLog ultraLogLog = getUltraLogLog(aggregationResultHolder);
    switch (storedType) {
      case INT:
        int[] intValues = blockValSet.getIntValuesSV();
        for (int i = 0; i < length; i++) {
          wyHashObject(intValues[i]).ifPresent(ultraLogLog::add);
        }
        break;
      case LONG:
        long[] longValues = blockValSet.getLongValuesSV();
        for (int i = 0; i < length; i++) {
          wyHashObject(longValues[i]).ifPresent(ultraLogLog::add);
        }
        break;
      case FLOAT:
        float[] floatValues = blockValSet.getFloatValuesSV();
        for (int i = 0; i < length; i++) {
          wyHashObject(floatValues[i]).ifPresent(ultraLogLog::add);
        }
        break;
      case DOUBLE:
        double[] doubleValues = blockValSet.getDoubleValuesSV();
        for (int i = 0; i < length; i++) {
          wyHashObject(doubleValues[i]).ifPresent(ultraLogLog::add);
        }
        break;
      case STRING:
        String[] stringValues = blockValSet.getStringValuesSV();
        for (int i = 0; i < length; i++) {
          wyHashObject(stringValues[i]).ifPresent(ultraLogLog::add);
        }
        break;
      default:
  ...
  public void aggregateGroupBySV(int length, int[] groupKeyArray, GroupByResultHolder groupByResultHolder,
    ...
    if (storedType == DataType.BYTES) {
      ...
      try {
        for (int i = 0; i < length; i++) {
          ...
          UltraLogLog ultraLogLog = groupByResultHolder.getResult(groupKey);
          if (ultraLogLog != null) {
            ultraLogLog.add(value);
          } else {
    ...
    switch (storedType) {
      case INT:
        int[] intValues = blockValSet.getIntValuesSV();
        for (int i = 0; i < length; i++) {
          wyHashObject(intValues[i]).ifPresent(getUltraLogLog(groupByResultHolder, groupKeyArray[i])::add);
        }
        break;
      case LONG:
        long[] longValues = blockValSet.getLongValuesSV();
        for (int i = 0; i < length; i++) {
          wyHashObject(longValues[i]).ifPresent(getUltraLogLog(groupByResultHolder, groupKeyArray[i])::add);
        }
        break;
      case FLOAT:
        float[] floatValues = blockValSet.getFloatValuesSV();
        for (int i = 0; i < length; i++) {
          wyHashObject(floatValues[i]).ifPresent(getUltraLogLog(groupByResultHolder, groupKeyArray[i])::add);
        }
        break;
      case DOUBLE:
        double[] doubleValues = blockValSet.getDoubleValuesSV();
        for (int i = 0; i < length; i++) {
          wyHashObject(doubleValues[i]).ifPresent(getUltraLogLog(groupByResultHolder, groupKeyArray[i])::add);
        }
        break;
      case STRING:
        String[] stringValues = blockValSet.getStringValuesSV();
        for (int i = 0; i < length; i++) {
          wyHashObject(stringValues[i]).ifPresent(getUltraLogLog(groupByResultHolder, groupKeyArray[i])::add);
        }
        break;
      default:
        ...
        for (int i = 0; i < length; i++) {
          ...
          for (int groupKey : groupKeysArray[i]) {
            ...
            if (ultraLogLog != null) {
              ...
              // Create a new UltraLogLog for the group
  ...
  public UltraLogLog extractAggregationResult(AggregationResultHolder aggregationResultHolder) {
    ...
    if (result == null) {
      return UltraLogLog.create(_log2m);
    }

    if (result instanceof DictIdsWrapper) {
      // For dictionary-encoded expression, convert dictionary ids to UltraLogLog
      return convertToUltraLogLog((DictIdsWrapper) result);
    } else {
      // For non-dictionary-encoded expression, directly return the UltraLogLog
  ...
  /**
   * Returns the UltraLogLog from the result holder or creates a new one if it does not exist.
   */
  protected UltraLogLog getUltraLogLog(AggregationResultHolder aggregationResultHolder) {
    UltraLogLog hyperLogLog = aggregationResultHolder.getResult();
    if (hyperLogLog == null) {
      hyperLogLog = UltraLogLog.create(_log2m);
   ...
   */
  protected UltraLogLog getUltraLogLog(GroupByResultHolder groupByResultHolder, int groupKey) {
    UltraLogLog hyperLogLog = groupByResultHolder.getResult(groupKey);
    if (hyperLogLog == null) {
      hyperLogLog = UltraLogLog.create(_log2m);
  ...
  private void setValueForGroupKeys(GroupByResultHolder groupByResultHolder, int[] groupKeys, Object value) {
    for (int groupKey : groupKeys) {
      wyHashObject(value).ifPresent(getUltraLogLog(groupByResultHolder, groupKey)::add);
    }
  }

  /**
   * Helper method to read dictionary and convert dictionary ids to UltraLogLog for dictionary-encoded expression.
   */
  private UltraLogLog convertToUltraLogLog(DictIdsWrapper dictIdsWrapper) {
    UltraLogLog ultraLogLog = UltraLogLog.create(_log2m);
    Dictionary dictionary = dictIdsWrapper._dictionary;
    RoaringBitmap dictIdBitmap = dictIdsWrapper._dictIdBitmap;
    PeekableIntIterator iterator = dictIdBitmap.getIntIterator();
    while (iterator.hasNext()) {
      wyHashObject(dictionary.get(iterator.next())).ifPresent(ultraLogLog::add);
    }
    return ultraLogLog;
  }
